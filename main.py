...
FAST_SALE_HEADER_EMOJI_ID = "5877613700344450910"
FREE_POSTING_HEADER_EMOJI_ID = "5875082500023258804"
BUY_ACCOUNT_HEADER_EMOJI_ID = "5875082500023258804"
REQUEST_ACCOUNT_HEADER_EMOJI_ID = "5875082500023258804"

# Inline button custom emoji ids (leave blank to disable)
INLINE_SUPPORT_EMOJI_ID = "5832251986635920010"
INLINE_TOTAL_ORDERS_EMOJI_ID = "5956561916573782596"
INLINE_CONFIRM_EMOJI_ID = "5951665890079544884"
INLINE_CANCEL_EMOJI_ID = "5877341274863832725"
INLINE_APPROVE_EMOJI_ID = "5931409969613116639"
INLINE_REJECT_EMOJI_ID = "5775887550262546277"
INLINE_REGISTER_ACCOUNT_EMOJI_ID = "5936017305585586269"
INLINE_MY_LISTINGS_EMOJI_ID = "5967548335542767952"
INLINE_CLOSE_EMOJI_ID = "5793933761594789855"
INLINE_BUY_CRYPTO_EMOJI_ID = "5870478797593120516"
INLINE_BUY_GIFTCARD_EMOJI_ID = "6032937473162614352"
INLINE_LANGUAGE_ICON_IDS = {
    "lang_zh": "",
    "lang_hi": "",
    "lang_ru": "",
    "lang_fa": "",
    ...
    return InlineKeyboardMarkup(
        [
            [
                inline_button(
                    "➕ Register Account",
                    callback_data=ADMIN_MENU_REGISTER,
                    icon_custom_emoji_id=INLINE_REGISTER_ACCOUNT_EMOJI_ID,
                ),
                inline_button(
                    "📄 My Listings",
                    callback_data=ADMIN_MENU_LIST,
                    icon_custom_emoji_id=INLINE_MY_LISTINGS_EMOJI_ID,
                ),
            ],
            [
                inline_button(
                    "❌ Close",
                    callback_data=ADMIN_MENU_CLOSE,
                    icon_custom_emoji_id=INLINE_CLOSE_EMOJI_ID,
                ),
            ],
        ]
    )
...
def build_buy_payment_keyboard(context: ContextTypes.DEFAULT_TYPE | None = None) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                inline_button(
                    tr(context, "USDT BEP20"),
                    callback_data=BUY_PAY_CRYPTO,
                    icon_custom_emoji_id=INLINE_BUY_CRYPTO_EMOJI_ID,
                ),
                inline_button(
                    tr(context, "Binance Gift Card"),
                    callback_data=BUY_PAY_GIFTCARD,
                    icon_custom_emoji_id=INLINE_BUY_GIFTCARD_EMOJI_ID,
                ),
            ]
        ]
    )
...
def main_menu_keyboard(context: ContextTypes.DEFAULT_TYPE | None = None) -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(
        [
            [menu_button(labels[MENU_FAST_SALE], MENU_ICON_FAST_SALE, KeyboardButtonStyle.DANGER)],
            [
                menu_button(labels[MENU_AUTO_AD], MENU_ICON_AUTO_AD, KeyboardButtonStyle.SUCCESS),
                menu_button(labels[MENU_REQUEST_ACCOUNT], MENU_ICON_REQUEST_ACCOUNT),
            ],
            [
                menu_button(labels[MENU_SUPPORT], MENU_ICON_SUPPORT, KeyboardButtonStyle.PRIMARY),
                menu_button(labels[MENU_ACCOUNT], MENU_ICON_ACCOUNT),
                menu_button(labels[MENU_LANGUAGE], MENU_ICON_LANGUAGE),
            ],
            [
                menu_button(labels[MENU_BUY_ACCOUNT], "", KeyboardButtonStyle.PRIMARY),
...
def ad_confirm_keyboard(context: ContextTypes.DEFAULT_TYPE | None = None) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                inline_button(
                    tr(context, "Confirm & Send"),
                    callback_data=AD_CONFIRM_YES,
                    style=KeyboardButtonStyle.SUCCESS,
                    icon_custom_emoji_id=INLINE_CONFIRM_EMOJI_ID,
                ),
                inline_button(
                    tr(context, "Cancel"),
                    callback_data=AD_CONFIRM_NO,
                    style=KeyboardButtonStyle.DANGER,
                    icon_custom_emoji_id=INLINE_CANCEL_EMOJI_ID,
                ),
            ]
        ]
    )
...
def buy_account_prompt_text(context: ContextTypes.DEFAULT_TYPE | None = None) -> tuple[str, list[MessageEntity]]:
    text = f"{PLACEHOLDER} {tr(context, 'Send the account number in this format: #account_number1987')}"
    return text, build_custom_emoji_entities(text, [BUY_ACCOUNT_HEADER_EMOJI_ID])
    ...
    return InlineKeyboardMarkup(
        [
            [
                inline_button(
                    "✅ Approve",
                    callback_data=f"{AD_APPROVE_PREFIX}{request_id}",
                    style=KeyboardButtonStyle.SUCCESS,
                    icon_custom_emoji_id=INLINE_APPROVE_EMOJI_ID,
                ),
                inline_button(
                    "❌ Reject",
                    callback_data=f"{AD_REJECT_PREFIX}{request_id}",
                    style=KeyboardButtonStyle.DANGER,
                    icon_custom_emoji_id=INLINE_REJECT_EMOJI_ID,
                ),
            ]
        ]
    )
    ...
    return InlineKeyboardMarkup(
        [
            [
                inline_button(
                    "✅ Approve",
                    callback_data=f"{REQ_APPROVE_PREFIX}{request_id}",
                    style=KeyboardButtonStyle.SUCCESS,
                    icon_custom_emoji_id=INLINE_APPROVE_EMOJI_ID,
                ),
                inline_button(
                    "❌ Reject",
                    callback_data=f"{REQ_REJECT_PREFIX}{request_id}",
                    style=KeyboardButtonStyle.DANGER,
                    icon_custom_emoji_id=INLINE_REJECT_EMOJI_ID,
                ),
            ]
        ...
        [
            [
                inline_button(
                    ...
                    icon_custom_emoji_id=INLINE_SUPPORT_EMOJI_ID,
                ),
            ]
        ]
    )
    ...
    return InlineKeyboardMarkup(
        [
            [
                ...
                inline_button(
                    tr(context, "Total Orders"),
                    callback_data=ACCOUNT_TOTAL_ORDERS,
                    icon_custom_emoji_id=INLINE_TOTAL_ORDERS_EMOJI_ID,
                ),
            ]
        ]
    )
...
def language_inline_keyboard(context: ContextTypes.DEFAULT_TYPE | None = None) -> InlineKeyboardMarkup:
    selected = (context.user_data.get("language_code") if context else None) or ""
    return InlineKeyboardMarkup(
        [
            [
                inline_button(
                    ...
                    style=KeyboardButtonStyle.SUCCESS if selected == LANG_ZH else None,
                    icon_custom_emoji_id=INLINE_LANGUAGE_ICON_IDS.get(LANG_ZH, ""),
                ),
                inline_button(
                    tr(context, "Hindi 🇮🇳"),
                    callback_data=LANG_HI,
                    style=KeyboardButtonStyle.SUCCESS if selected == LANG_HI else None,
                    icon_custom_emoji_id=INLINE_LANGUAGE_ICON_IDS.get(LANG_HI, ""),
                ),
            ],
            [
                inline_button(
                    ...
                    style=KeyboardButtonStyle.SUCCESS if selected == LANG_RU else None,
                    icon_custom_emoji_id=INLINE_LANGUAGE_ICON_IDS.get(LANG_RU, ""),
                ),
                inline_button(
                    tr(context, "Farsi 🇮🇷"),
                    callback_data=LANG_FA,
                    style=KeyboardButtonStyle.SUCCESS if selected == LANG_FA else None,
                    icon_custom_emoji_id=INLINE_LANGUAGE_ICON_IDS.get(LANG_FA, ""),
                ),
            ],
            [
                inline_button(
                    ...
                    style=KeyboardButtonStyle.SUCCESS if selected == LANG_EN else None,
                    icon_custom_emoji_id=INLINE_LANGUAGE_ICON_IDS.get(LANG_EN, ""),
                ),
                inline_button(
                    tr(context, "Arabic 🇸🇦"),
                    callback_data=LANG_AR,
                    style=KeyboardButtonStyle.SUCCESS if selected == LANG_AR else None,
                    icon_custom_emoji_id=INLINE_LANGUAGE_ICON_IDS.get(LANG_AR, ""),
                ),
            ],
            [
                inline_button(
                    ...
                    style=KeyboardButtonStyle.SUCCESS if selected == LANG_HE else None,
                    icon_custom_emoji_id=INLINE_LANGUAGE_ICON_IDS.get(LANG_HE, ""),
                ),
                inline_button(
                    tr(context, "Uzbekcha 🇺🇿"),
                    callback_data=LANG_UZ,
                    style=KeyboardButtonStyle.SUCCESS if selected == LANG_UZ else None,
                    icon_custom_emoji_id=INLINE_LANGUAGE_ICON_IDS.get(LANG_UZ, ""),
                ),
            ],
            [
                inline_button(
                    ...
                    style=KeyboardButtonStyle.SUCCESS if selected == LANG_IT else None,
                    icon_custom_emoji_id=INLINE_LANGUAGE_ICON_IDS.get(LANG_IT, ""),
                ),
                inline_button(
                    tr(context, "Turkce 🇹🇷"),
                    callback_data=LANG_TR,
                    style=KeyboardButtonStyle.SUCCESS if selected == LANG_TR else None,
                    icon_custom_emoji_id=INLINE_LANGUAGE_ICON_IDS.get(LANG_TR, ""),
                ),
            ],
            [
                inline_button(
                    ...
                    style=KeyboardButtonStyle.SUCCESS if selected == LANG_FR else None,
                    icon_custom_emoji_id=INLINE_LANGUAGE_ICON_IDS.get(LANG_FR, ""),
                ),
                inline_button(
                    tr(context, "Spanish 🇪🇸"),
                    callback_data=LANG_ES,
                    style=KeyboardButtonStyle.SUCCESS if selected == LANG_ES else None,
                    icon_custom_emoji_id=INLINE_LANGUAGE_ICON_IDS.get(LANG_ES, ""),
                ),
            ],
        ]
    )
...
async def handle_menu_shortcuts(update: Update, context: ContextTypes.DEFAULT_TYPE) -> Optional[int]:
    ...
    if button_text_matches(context, text, MENU_FAST_SALE):
        ...
        fast_sale_text = f"{PLACEHOLDER} {fast_sale_text}"
        await msg.reply_text(
            fast_sale_text,
            entities=build_custom_emoji_entities(fast_sale_text, [FAST_SALE_HEADER_EMOJI_ID]),
    ...
    if button_text_matches(context, text, MENU_AUTO_AD):
        reset_flow(context)
        free_posting_text = f"{PLACEHOLDER} {tr(context, 'Send a short video of the account. Maximum 2 minutes.')}"
        await msg.reply_text(
            free_posting_text,
            entities=build_custom_emoji_entities(free_posting_text, [FREE_POSTING_HEADER_EMOJI_ID]),
    ...
    if button_text_matches(context, text, MENU_REQUEST_ACCOUNT):
        reset_flow(context)
        request_account_text = f"{PLACEHOLDER} {tr(context, 'Type the skins and guns you want inside the account.')}"
        await msg.reply_text(
            request_account_text,
            entities=build_custom_emoji_entities(request_account_text, [REQUEST_ACCOUNT_HEADER_EMOJI_ID]),
            reply_markup=nav_keyboard(context),
        )
    ...
    if button_text_matches(context, text, MENU_BUY_ACCOUNT):
        ...
        buy_account_text = f"{PLACEHOLDER} {tr(context, 'Send the account number in this format: #account_number1987')}"
        await msg.reply_text(
            buy_account_text,
            entities=build_custom_emoji_entities(buy_account_text, [BUY_ACCOUNT_HEADER_EMOJI_ID]),
            reply_markup=nav_keyboard(context),
        )
        return MAIN_MENU
