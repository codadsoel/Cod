...
# Button styles
MENU_STYLE_FAST_SALE = KeyboardButtonStyle.DANGER
MENU_STYLE_AUTO_AD = KeyboardButtonStyle.SUCCESS
MENU_STYLE_SUPPORT = KeyboardButtonStyle.PRIMARY
MENU_STYLE_DEFAULT = KeyboardButtonStyle.PRIMARY
...
def build_admin_panel_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("➕ Register Account", callback_data=ADMIN_MENU_REGISTER, style=KeyboardButtonStyle.SUCCESS),
                InlineKeyboardButton("📄 My Listings", callback_data=ADMIN_MENU_LIST, style=KeyboardButtonStyle.PRIMARY),
            ],
            [
                InlineKeyboardButton("❌ Close", callback_data=ADMIN_MENU_CLOSE, style=KeyboardButtonStyle.DANGER),
            ],
        ]
    )
...
def build_buy_payment_keyboard(context: ContextTypes.DEFAULT_TYPE | None = None) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(tr(context, "USDT BEP20"), callback_data=BUY_PAY_CRYPTO, style=KeyboardButtonStyle.SUCCESS),
                InlineKeyboardButton(tr(context, "Binance Gift Card"), callback_data=BUY_PAY_GIFTCARD, style=KeyboardButtonStyle.PRIMARY),
            ]
        ]
    )
...
def build_buy_order_admin_keyboard(order_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("✅ Approve", callback_data=f"{BUY_APPROVE_PREFIX}{order_id}", style=KeyboardButtonStyle.SUCCESS),
                InlineKeyboardButton("❌ Reject", callback_data=f"{BUY_REJECT_PREFIX}{order_id}", style=KeyboardButtonStyle.DANGER),
            ]
        ]
    )
...
def menu_button(
    ...
    style: str = KeyboardButtonStyle.PRIMARY,
) -> KeyboardButton:
...
def main_menu_keyboard(context: ContextTypes.DEFAULT_TYPE | None = None) -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(
        [
            [menu_button(labels[MENU_FAST_SALE], MENU_ICON_FAST_SALE, MENU_STYLE_FAST_SALE)],
            [
                menu_button(labels[MENU_AUTO_AD], MENU_ICON_AUTO_AD, MENU_STYLE_AUTO_AD),
                menu_button(labels[MENU_REQUEST_ACCOUNT], MENU_ICON_REQUEST_ACCOUNT, KeyboardButtonStyle.PRIMARY),
            ],
            [
                menu_button(labels[MENU_SUPPORT], MENU_ICON_SUPPORT, MENU_STYLE_SUPPORT),
                menu_button(labels[MENU_ACCOUNT], MENU_ICON_ACCOUNT, KeyboardButtonStyle.PRIMARY),
                menu_button(labels[MENU_LANGUAGE], MENU_ICON_LANGUAGE, KeyboardButtonStyle.PRIMARY),
            ],
            [
                menu_button(labels[MENU_BUY_ACCOUNT], "", KeyboardButtonStyle.PRIMARY),
                menu_button(labels[MENU_MY_LISTINGS], "", KeyboardButtonStyle.PRIMARY),
                menu_button(labels[MENU_SELLER_RATING], "", KeyboardButtonStyle.PRIMARY),
            ],
        ],
...
def nav_keyboard(context: ContextTypes.DEFAULT_TYPE | None = None) -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(
        [
            [menu_button(labels[BACK_MENU_TEXT], "", KeyboardButtonStyle.PRIMARY)],
            [menu_button(labels[CANCEL_TEXT], "", KeyboardButtonStyle.DANGER)],
        ],
        resize_keyboard=True,
        one_time_keyboard=False,
    )
...
def ad_confirm_keyboard(context: ContextTypes.DEFAULT_TYPE | None = None) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(tr(context, "Confirm & Send"), callback_data=AD_CONFIRM_YES, style=KeyboardButtonStyle.SUCCESS),
                InlineKeyboardButton(tr(context, "Cancel"), callback_data=AD_CONFIRM_NO, style=KeyboardButtonStyle.DANGER),
            ]
        ]
    )
...
def sale_plan_keyboard(context: ContextTypes.DEFAULT_TYPE | None = None) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton(tr(context, "5 USD Package"), callback_data=PLAN_5, style=KeyboardButtonStyle.PRIMARY)],
            [InlineKeyboardButton(tr(context, "10 USD Package"), callback_data=PLAN_10, style=KeyboardButtonStyle.PRIMARY)],
        ]
    )
...
def payment_method_keyboard(context: ContextTypes.DEFAULT_TYPE | None = None) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton(tr(context, "USDT BEP20"), callback_data=PAY_CRYPTO, style=KeyboardButtonStyle.SUCCESS)],
            [InlineKeyboardButton(tr(context, "Binance Gift Card"), callback_data=PAY_GIFTCARD, style=KeyboardButtonStyle.PRIMARY)],
        ]
    )
...
def request_confirm_keyboard(context: ContextTypes.DEFAULT_TYPE | None = None) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(tr(context, "Send to Admin"), callback_data=REQ_CONFIRM_YES, style=KeyboardButtonStyle.SUCCESS),
                InlineKeyboardButton(tr(context, "Cancel"), callback_data=REQ_CONFIRM_NO, style=KeyboardButtonStyle.DANGER),
            ]
        ]
    )
...
def build_admin_ad_review_keyboard(request_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("✅ Approve", callback_data=f"{AD_APPROVE_PREFIX}{request_id}", style=KeyboardButtonStyle.SUCCESS),
                InlineKeyboardButton("❌ Reject", callback_data=f"{AD_REJECT_PREFIX}{request_id}", style=KeyboardButtonStyle.DANGER),
            ]
        ]
    )
...
def build_admin_request_review_keyboard(request_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("✅ Approve", callback_data=f"{REQ_APPROVE_PREFIX}{request_id}", style=KeyboardButtonStyle.SUCCESS),
                InlineKeyboardButton("❌ Reject", callback_data=f"{REQ_REJECT_PREFIX}{request_id}", style=KeyboardButtonStyle.DANGER),
            ]
        ]
    )
...
def support_inline_keyboard(context: ContextTypes.DEFAULT_TYPE | None = None) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(tr(context, "Support"), url=SUPPORT_CONTACT_URL, style=KeyboardButtonStyle.PRIMARY),
            ]
        ]
    )
...
def account_inline_keyboard(context: ContextTypes.DEFAULT_TYPE | None = None) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(tr(context, "Support"), url=SUPPORT_CONTACT_URL, style=KeyboardButtonStyle.PRIMARY),
                InlineKeyboardButton(tr(context, "Total Orders"), callback_data=ACCOUNT_TOTAL_ORDERS, style=KeyboardButtonStyle.SUCCESS),
            ]
        ]
    )
...
def language_inline_keyboard(context: ContextTypes.DEFAULT_TYPE | None = None) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(tr(context, "Chinese 🇨🇳"), callback_data=LANG_ZH, style=KeyboardButtonStyle.PRIMARY),
                InlineKeyboardButton(tr(context, "Hindi 🇮🇳"), callback_data=LANG_HI, style=KeyboardButtonStyle.PRIMARY),
            ],
            [
                InlineKeyboardButton(tr(context, "Russian 🇷🇺"), callback_data=LANG_RU, style=KeyboardButtonStyle.PRIMARY),
                InlineKeyboardButton(tr(context, "Farsi 🇮🇷"), callback_data=LANG_FA, style=KeyboardButtonStyle.PRIMARY),
            ],
            [
                InlineKeyboardButton(tr(context, "English 🇺🇸"), callback_data=LANG_EN, style=KeyboardButtonStyle.PRIMARY),
                InlineKeyboardButton(tr(context, "Arabic 🇸🇦"), callback_data=LANG_AR, style=KeyboardButtonStyle.PRIMARY),
            ],
            [
                InlineKeyboardButton(tr(context, "Hebrew 🇮🇱"), callback_data=LANG_HE, style=KeyboardButtonStyle.PRIMARY),
                InlineKeyboardButton(tr(context, "Uzbekcha 🇺🇿"), callback_data=LANG_UZ, style=KeyboardButtonStyle.PRIMARY),
            ],
            [
                InlineKeyboardButton(tr(context, "Italiano 🇮🇹"), callback_data=LANG_IT, style=KeyboardButtonStyle.PRIMARY),
                InlineKeyboardButton(tr(context, "Turkce 🇹🇷"), callback_data=LANG_TR, style=KeyboardButtonStyle.PRIMARY),
            ],
            [
                InlineKeyboardButton(tr(context, "French 🇫🇷"), callback_data=LANG_FR, style=KeyboardButtonStyle.PRIMARY),
                InlineKeyboardButton(tr(context, "Spanish 🇪🇸"), callback_data=LANG_ES, style=KeyboardButtonStyle.PRIMARY),
            ],
        ]
    )
